...
class ExcelExporter:
    ...
    def __init__(self, data_frames, schedule_generator):
        ...
        classroom_inventory = self._extract_classroom_inventory()
        self._all_classrooms = classroom_inventory['all']
        self._nonlab_classrooms = classroom_inventory['nonlab']
    ...
    def _extract_classroom_inventory(self):
        """Build lists of available classrooms, prioritizing non-lab rooms."""
        inventory = {'all': [], 'nonlab': []}
        ...
        for _, row in cls_df.iterrows():
            ...
            inventory['all'].append((room_name, capacity))
            if 'lab' not in room_type:
                inventory['nonlab'].append((room_name, capacity))
        # Sort for determinism (prefer larger rooms later so smaller ones used first)
        inventory['all'].sort(key=lambda x: (x[1], x[0]))
        inventory['nonlab'].sort(key=lambda x: (x[1], x[0]))
        return inventory
    
    def _assign_basket_rooms(self, basket_codes):
        """Assign unique classrooms to each basket (non-lab rooms only)."""
        if not basket_codes:
            return {}
        room_pool = self._nonlab_classrooms or self._all_classrooms
        if not room_pool:
            print("    WARNING: No classroom data available; basket slots will not show rooms.")
            return {code: '' for code in basket_codes}
        
        unique_rooms = []
        seen = set()
        for room_name, _ in room_pool:
            if room_name not in seen:
                unique_rooms.append(room_name)
                seen.add(room_name)
        assignments = {}
        for idx, code in enumerate(basket_codes):
            if idx < len(unique_rooms):
                assignments[code] = unique_rooms[idx]
            else:
                # Reuse starting from first room but warn user
                assignments[code] = unique_rooms[idx % len(unique_rooms)]
        if len(unique_rooms) < len(basket_codes):
            print("    WARNING: Not enough unique classrooms for all baskets; some rooms will repeat.")
        print(f"    Basket classroom assignments: {assignments}")
        return assignments
    
    def _assign_course_rooms_within_baskets(self, basket_assignments):
        """Ensure every course inside a basket is placed in a distinct classroom."""
        if basket_assignments.empty:
            basket_assignments['Classroom'] = ''
            return basket_assignments
        
        room_pool = self._nonlab_classrooms or self._all_classrooms
        if not room_pool:
            print("    WARNING: No classrooms available to allocate per-course rooms inside baskets.")
            basket_assignments['Classroom'] = ''
            return basket_assignments
        
        room_names = [room for room, _ in room_pool]
        if not room_names:
            print("    WARNING: Classroom list is empty; cannot assign per-course rooms.")
            basket_assignments['Classroom'] = ''
            return basket_assignments
        
        updated = basket_assignments.copy()
        grouped = updated.groupby('Basket Code', sort=False)
        for basket_code, group in grouped:
            indices = group.index.tolist()
            needed = len(indices)
            if needed == 0:
                continue
            if len(room_names) < needed:
                print(f"    WARNING: Basket {basket_code} needs {needed} classrooms but only {len(room_names)} available; some rooms will repeat.")
            used_rooms = set()
            room_iter = 0
            for idx in indices:
                # Find next room not already used for this basket
                attempts = 0
                room_name = ''
                # Try to find an unused room first
                while attempts < len(room_names):
                    candidate = room_names[room_iter % len(room_names)]
                    room_iter += 1
                    attempts += 1
                    # If we haven't used all rooms yet, only accept unused ones
                    if len(used_rooms) < len(room_names):
                        if candidate not in used_rooms:
                            room_name = candidate
                            used_rooms.add(candidate)
                            break
                    else:
                        # All rooms used, can reuse any (cycle through)
                        room_name = candidate
                        used_rooms.add(candidate)
                        break
                # If somehow no room found, use first available
                if not room_name and room_names:
                    room_name = room_names[0]
                    used_rooms.add(room_name)
                updated.at[idx, 'Classroom'] = room_name
        return updated
    ...
    def _create_elective_classrooms_sheet(self, semester):
        ...
        try:
            ...
            elective_sheet_key = None
            for key in self.dfs.keys():
                key_lower = key.lower()
                # Match patterns like "course_elective_data", "elective_data", etc.
                if 'elective' in key_lower and 'data' in key_lower:
                    elective_sheet_key = key
                    break
            
            if elective_sheet_key is None:
                print(f"    WARNING: 'Elective data' sheet not found in course_data.xlsx")
                return pd.DataFrame()
            
            elective_df_raw = self.dfs[elective_sheet_key]
            if elective_df_raw.empty:
                print(f"    INFO: 'Elective data' sheet is empty")
                return pd.DataFrame()
            
            print(f"    INFO: Reading from '{elective_sheet_key}' sheet")
            
            # Filter for the semester
            elective_df = elective_df_raw.copy()
            
            # Find Semester column (case-insensitive)
            semester_col = None
            for col in elective_df.columns:
                if 'semester' in str(col).lower():
                    semester_col = col
                    break
            
            if semester_col:
                elective_df[semester_col] = pd.to_numeric(elective_df[semester_col], errors='coerce')
                sem_electives = elective_df[elective_df[semester_col] == semester].copy()
            else:
                print(f"    WARNING: No Semester column found in Elective data sheet")
                return pd.DataFrame()
            
            if sem_electives.empty:
                print(f"    INFO: No electives found for semester {semester} in Elective data sheet")
                return pd.DataFrame()
            
            print(f"    INFO: Found {len(sem_electives)} elective(s) for semester {semester}")
            ...
            print(f"    ERROR creating elective classrooms sheet: {e}")
            import traceback
            traceback.print_exc()
            return pd.DataFrame()
    
    def _create_minor_classrooms_sheet(self, semester):
        """Create a sheet with minor courses and their assigned classrooms.
        Classrooms are assigned based on student registration and capacity.
        Reads from 'Minor data' sheet in course_data.xlsx.
        
        Args:
            semester: Semester number (3 or 5)
        
        Returns:
            DataFrame with columns from Minor data sheet plus Classroom and Capacity
        """
        try:
            # Look for "Minor data" sheet in data frames
            minor_sheet_key = None
            for key in self.dfs.keys():
                key_lower = key.lower()
                # Match patterns like "course_minor_data", "minor_data", etc.
                if 'minor' in key_lower and 'data' in key_lower:
                    minor_sheet_key = key
                    break
            
            if minor_sheet_key is None:
                print(f"    WARNING: 'Minor data' sheet not found in course_data.xlsx")
                return pd.DataFrame()
            
            minor_df_raw = self.dfs[minor_sheet_key]
            if minor_df_raw.empty:
                print(f"    INFO: 'Minor data' sheet is empty")
                return pd.DataFrame()
            
            print(f"    INFO: Reading from '{minor_sheet_key}' sheet")
            
            # Filter for the semester
            minor_df = minor_df_raw.copy()
            ...
            if semester_col:
                minor_df[semester_col] = pd.to_numeric(minor_df[semester_col], errors='coerce')
                sem_minors = minor_df[minor_df[semester_col] == semester].copy()
            else:
                print(f"    WARNING: No Semester column found in Minor data sheet")
                return pd.DataFrame()
            
            if sem_minors.empty:
                print(f"    INFO: No minor courses found for semester {semester} in Minor data sheet")
                return pd.DataFrame()
            
            print(f"    INFO: Found {len(sem_minors)} minor course(s) for semester {semester}")
            
            # Prepare result with all columns from Minor data sheet
            result_df = sem_minors.copy()
            
            # Get all non-lab classrooms sorted by capacity (ascending - prefer smaller rooms first)
            all_rooms = sorted(self._nonlab_classrooms or self._all_classrooms, key=lambda x: x[1] if x[1] > 0 else 9999)
            
            if not all_rooms:
                print(f"    WARNING: No classrooms available for minor courses in semester {semester}")
                result_df['Classroom'] = ''
                result_df['Capacity'] = 0
                return result_df
            ...
            if not result_df.empty:
                print(f"    Assigned {len(result_df)} minor classrooms for semester {semester} based on student registration")
            
            return result_df
            
        except Exception as e:
            print(f"    ERROR creating minor classrooms sheet: {e}")
            import traceback
            traceback.print_exc()
            return pd.DataFrame()
    ...
    def export_semester_timetable(self, semester):
        ...
        try:
            with writer as w:
                ...
                # Add Elective Classrooms sheet for semesters 1, 3, 5
                if semester in [1, 3, 5]:
                    try:
                        ...
                        print(f"  WARNING: Could not create Elective Classrooms sheet: {e}")
                
                # Add Minor Classrooms sheet for semesters 3, 5
                if semester in [3, 5]:
                    try:
                        print(f"\n  Creating Minor Classrooms sheet for semester {semester}...")
    ...
    def export_semester7_timetable(self):
        ...
        try:
            with writer as w:
                ...
                basket_room_map = self._assign_basket_rooms(basket_codes)
                if basket_room_map:
                    for day in DAYS:
                        for slot in sem7_slots:
                            val = schedule.loc[day, slot]
                            if val in basket_room_map and basket_room_map[val]:
                                schedule.loc[day, slot] = f"{val} ({basket_room_map[val]})"
                ...
                if not basket_assignments.empty:
