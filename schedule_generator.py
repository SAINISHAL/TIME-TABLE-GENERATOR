...
class ScheduleGenerator:
    ...
    def _is_time_slot_available_global(self, day, slots, department, session, semester_id, allow_combined=False, paired_dept=None):
        ...
        - Combined classes: paired departments (CSE-A/CSE-B, ECE/DSAI) can share slots"""
        ...
        for slot in slots:
            ...
            for dept_key, used_slots in self.semester_global_slots[semester_key].items():
                if (day, slot) in used_slots:
                    ...
                    # Allow combined classes: paired departments can share slots
                    if allow_combined and paired_dept:
                        if dept_in_slot == paired_dept and session == session_in_slot:
                            continue  # Allow sharing for combined classes
    ...
    def _schedule_lectures(self, schedule, course_code, lectures_per_week, department, session, semester_id, avoid_days=None, is_combined=False, paired_dept=None, group_key=None):
        """Schedule lecture sessions, returns list of (day, slot) tuples.
        
        Args:
            is_combined: Whether this is a combined class
            paired_dept: The paired department for combined classes
            group_key: The combined group key (e.g., 'CSE' or 'DSAI_ECE')
        """
        ...
        if is_combined and paired_dept and group_key:
            ...
            sem_key = (semester_id, course_key, 'Lecture')
            global_key = ('GLOBAL', group_key, course_key, 'Lecture')
            
            # Check if slots already exist for this combined class
            existing_slots = []
            if sem_key in self.semester_combined_course_slots:
                existing_slots = self.semester_combined_course_slots[sem_key]
            elif global_key in self.global_combined_course_slots:
                existing_slots = self.global_combined_course_slots[global_key]
            
            # Reuse existing slots if available
            if existing_slots:
                for day, start_slot in existing_slots:
                    if len(scheduled_slots) >= lectures_per_week * LECTURE_DURATION:
                        break
                    slots = self._get_consecutive_slots(start_slot, LECTURE_DURATION)
                    if (len(slots) == LECTURE_DURATION and
                        self._is_time_slot_available_local(schedule, day, slots)):
                        # Use existing combined class slots
                        for slot in slots:
        ...
        while len(scheduled_slots) < lectures_per_week * LECTURE_DURATION and attempts < max_attempts:
            ...
            # For combined classes, allow sharing with paired department
            slots_available = True
            for slot in slots:
                local_ok = self._is_time_slot_available_local(schedule, day, [slot])
                if is_combined and paired_dept:
                    # Combined class: allow if local is free and paired dept can share
                    global_ok = self._is_time_slot_available_global(day, [slot], department, session, semester_id, 
                                                                    allow_combined=True, paired_dept=paired_dept)
                else:
                    # Regular class: check normal global availability
                    global_ok = self._is_time_slot_available_global(day, [slot], department, session, semester_id)
            ...
            if slots_available:
                ...
                for slot in slots:
                    self._mark_slots_busy_local(schedule, day, [slot], course_code, 'Lecture')
                    # For combined classes, mark globally but allow both departments to share
                    # For regular classes, mark globally normally
                    self._mark_slots_busy_global(day, [slot], department, session, semester_id)
                    scheduled_slots.append((day, slot))
                used_days.add(day)
                avoid_days.add(day)
                
                # Store combined class slots for paired department to reuse
                if is_combined and paired_dept and group_key:
                    course_key = str(course_code).strip()
                    sem_key = (semester_id, course_key, 'Lecture')
                    global_key = ('GLOBAL', group_key, course_key, 'Lecture')
                    
                    # Store semester-specific
                    if sem_key not in self.semester_combined_course_slots:
                        self.semester_combined_course_slots[sem_key] = []
                    if (day, start_slot) not in self.semester_combined_course_slots[sem_key]:
                    ...
                    if (day, start_slot) not in self.global_combined_course_slots[global_key]:
    ...
    def _is_combined_class(self, course):
        """Check if a course is marked as a combined class from course data."""
        # Check for combined class indicators in various column names
        combined_cols = ['Combined Class', 'c004', 'C004', 'Combined', 'Is Combined', 'Is_Combined']
        for colname in combined_cols:
            if colname in course.index:
                val = str(course.get(colname, '')).strip().upper()
                if val in ['YES', 'Y', 'TRUE', '1', 'COMBINED', 'C004']:
                    return True
        return False
    
    def _get_paired_department(self, department):
        """Get the paired department for combined classes. Returns None if not part of a valid pair."""
        pairs = {
            'CSE-A': 'CSE-B',
            'CSE-B': 'CSE-A',
            'ECE': 'DSAI',
        ...
        return pairs.get(department, None)
    ...
    def _schedule_course(self, schedule, course, department, session, semester_id):
        ...
        is_combined = self._is_combined_class(course)
        paired_dept = self._get_paired_department(department) if is_combined else None
        ...
        if is_combined and paired_dept:
            print(f"      [COMBINED CLASS] {course_code} for {department} and {paired_dept}")
        ...
        def _apply_existing_combined(component_name, duration, required_count):
            ...
            # For combined classes, check if paired department has already scheduled this
            if is_combined and paired_dept and group_key:
                # Check if paired department has scheduled this course already
                paired_key = (semester_id, paired_dept, session, course_key)
                if paired_key in self.scheduled_courses:
                    # Look for slots scheduled by the paired department
                    # We need to find the actual slots from semester_combined_course_slots or global
                    # Try to find slots assigned to the paired department
                    paired_sem_key = (semester_id, course_key, component_name)
                    if paired_sem_key in self.semester_combined_course_slots:
                        assigned = self.semester_combined_course_slots[paired_sem_key]
                    else:
                        # Try global combined slots
                    ...
                    if assigned:
                        ...
                        for day, start_slot in assigned:
                            ...
                            # For combined classes, we must use the same slots even if they seem "busy"
                            # because both departments share the same time slot
                            if (len(slots) == duration and
                                self._is_time_slot_available_local(schedule, day, slots)):
                                # Mark slots locally but allow sharing with paired department globally
                                ...
                                # Mark globally but allow both departments in the pair
                                self._mark_slots_busy_global(day, slots, department, session, semester_id)
                                # Also store for later departments to reuse
                                if paired_sem_key not in self.semester_combined_course_slots:
                                    self.semester_combined_course_slots[paired_sem_key] = [(day, start_slot)]
                                elif (day, start_slot) not in self.semester_combined_course_slots[paired_sem_key]:
                                    self.semester_combined_course_slots[paired_sem_key].append((day, start_slot))
                                applied += 1
            ...
            # Standard combined slot lookup (existing logic)
            ...
            for day, start_slot in assigned:
                slots = self._get_consecutive_slots(start_slot, duration)
                # For combined classes, we need to allow both departments to use same slots
                slot_available = False
                if is_combined and paired_dept:
                    # Combined class: allow if local slot is free, even if paired dept has it globally
                    slot_available = self._is_time_slot_available_local(schedule, day, slots)
                    # Don't check global availability for combined classes - they share slots
                else:
                    # Regular class: check both local and global
                    slot_available = (self._is_time_slot_available_local(schedule, day, slots) and
                                     self._is_time_slot_available_global(day, slots, department, session, semester_id))
                
                if (len(slots) == duration and slot_available):
                    self._mark_slots_busy_local(schedule, day, slots, course_code, 'Lecture' if component_name=='Lecture' else ('Tutorial' if component_name=='Tutorial' else 'Lab'))
                    if not is_combined or not paired_dept:
                        # Only mark globally for non-combined classes
                        self._mark_slots_busy_global(day, slots, department, session, semester_id)
                    else:
                        # For combined classes, mark for both departments but allow sharing
                        # Store the slots so paired department can reuse them
                        if sem_key not in self.semester_combined_course_slots:
                            self.semester_combined_course_slots[sem_key] = []
                        if (day, start_slot) not in self.semester_combined_course_slots[sem_key]:
                            self.semester_combined_course_slots[sem_key].append((day, start_slot))
                            # Also store globally for this group
                            if global_key:
                                if global_key not in self.global_combined_course_slots:
                                    self.global_combined_course_slots[global_key] = []
                                if (day, start_slot) not in self.global_combined_course_slots[global_key]:
        ...
        if lectures_per_week > 0:
            if elective_flag:
                ...
                lec_slots = []
                if lectures_remaining > 0:
                    lec_slots = self._schedule_lectures(schedule, course_code, lectures_remaining, department, session, semester_id, 
                                                       avoid_days=avoid_days, is_combined=is_combined, paired_dept=paired_dept, group_key=group_key)
                success_counts['lectures'] = applied_lec_count + (len(lec_slots) // LECTURE_DURATION)
            scheduled_slots.extend(applied_lec_slots + lec_slots)
            
            # Store combined class slots for paired department to reuse
            if is_combined and paired_dept and group_key and lectures_remaining > 0 and lec_slots:
                course_key = str(course_code).strip()
                # Extract unique (day, start_slot) pairs from lec_slots
                slot_groups = {}
                for day, slot in lec_slots:
                    if day not in slot_groups:
                        slot_groups[day] = []
                    slot_groups[day].append(slot)
                # For each day, find the start slot (first slot in consecutive sequence)
                for day in slot_groups:
                    slots = sorted(slot_groups[day])
                    if slots:
                        # Find consecutive groups
                        start_slot = slots[0]
                        # Check if this is a valid lecture sequence
                        expected_slots = self._get_consecutive_slots(start_slot, LECTURE_DURATION)
                        if len(expected_slots) == LECTURE_DURATION and all(s in slots for s in expected_slots):
                            sem_key = (semester_id, course_key, 'Lecture')
                            if sem_key not in self.semester_combined_course_slots:
                                self.semester_combined_course_slots[sem_key] = []
                            if (day, start_slot) not in self.semester_combined_course_slots[sem_key]:
